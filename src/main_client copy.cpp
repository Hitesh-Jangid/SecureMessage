...
void processReceivedMessage(const std::string &receivedPacket, const std::string &recipientUsername) {
    ...
    // Use the hex-encoded encrypted payload (secMsg.encryptedPayload) for signature verification.
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            // Ensure the sender's username is included so they can decrypt their own message.
            ...
            // Prepare the encrypted AES keys for each recipient using their public key.
